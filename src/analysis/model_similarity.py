...
def compute_cka_by_gap(
    ...
    """
    Compute average CKA grouped by task gap using raw representations.

    For each pair of models with gap g, compute linear CKA,
    then aggregate CKA values for that gap to compute mean and std.
    """
    ...
    for i in range(start_idx, len(task_indices)):
        for j in range(i + 1, len(task_indices)):
            ...
            cka_val = compute_linear_cka(reps_dict[task_i], reps_dict[task_j])
...
def plot_cka_decay_profile(
    ...
    """
    Plot CKA decay profile for all layers on one figure.

    X-axis: Number of tasks between two models (gap)
    Y-axis: Average CKA for model pairs with that gap
    """
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    for layer_name in layer_names:
        reps_dict = all_reps[layer_name]
        gaps, means, stds = compute_cka_by_gap(reps_dict, task_indices, exclude_first)
        all_gaps.extend(gaps)
        ax.errorbar(gaps, means, yerr=stds, marker='o', capsize=5, label=layer_name)

    ax.set_title('CKA Decay Profile')
    ax.set_xlabel('Task Gap')
    ax.set_ylabel('CKA')
    ax.set_ylim(0, 1)
    ...
    plt.tight_layout()
    plt.savefig(output_path)
    print(f"CKA decay profile saved to {output_path}")
    plt.close()
...
def run_model_cka_similarity(
    ...
    """
    Generate model pairwise CKA matrices.

    For each layer, computes linear CKA between representations
    from different checkpoint models.
    """
    print("\nGenerating model CKA matrix heatmaps...")
    ...
    # Compute and plot CKA matrix for each layer
    for layer in layer_names:
        reps_list = [all_reps[layer][t] for t in sorted_task_indices]
        ...
        matrix_path = os.path.join(matrix_dir, f"cka_matrix_{safe_layer_name}.png")
    ...
    # Plot CKA decay profile for all layers on one figure
    profile_path = os.path.join(output_dir, "cka_decay_profile.png")
    plot_cka_decay_profile(all_reps, sorted_task_indices, layer_names, profile_path)

    print(f"All model CKA matrices saved to {matrix_dir}")
    print(f"CKA decay profile saved to {profile_path}")
