"""Reference-anchored STPV drift analysis module for RNN.

Computes drift metrics by comparing Spatiotemporal Population Vectors (STPVs)
from each checkpoint against the reference (first task) model.
...
def run_reference_drift(
    ...
    """
    Run reference-anchored STPV drift analysis.

    Compares STPVs from each checkpoint against the reference
    ...
    for probe_task in probe_tasks:
        ...
        reference = torch.from_numpy(reps_dict[reference_idx]).float()
        ...
        if neuron_ratio < 1.0:
            ...
            reference = reference[:, neuron_idx]
        ...
        for task_idx in sorted_indices:
            ...
            metrics = compute_metrics(reference, current)
            all_results.append({
    ...
    # Save metrics JSON
    metrics_path = os.path.join(output_dir, "reference_drift_metrics.json")
...
def run_baseline_drift(*args, **kwargs) -> None:
    """Compatibility alias for the old API name."""
    return run_reference_drift(*args, **kwargs)
